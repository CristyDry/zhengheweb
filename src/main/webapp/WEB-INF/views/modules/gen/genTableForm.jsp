					...
					<div class="control-group">
						<table id="contentTable" class="table table-striped table-bordered table-condensed">
							<thead><tr><th title="数据库字段名">列名</th><th title="默认读取数据库字段备注">说明</th><th title="数据库中设置的字段类型及长度">物理类型</th><th title="实体对象的属性字段类型">Java类型</th>
								...
								<th title="选中后该字段被加入接口入参">入参</th>
								<th title="选中后该字段被加入接口出参">出参</th>
							...
							<c:forEach items="${genTable.columnList}" var="column" varStatus="vs">
								<tr${column.delFlag eq '1'?' class="error" title="已删除的列，保存之后消失！"':''}>
									...
									<td>
										<input type="checkbox" name="columnList[${vs.index}].isReq" value="1" ${column.isReq eq '1' ? 'checked' : ''}/>
									</td>
									<td>
										<input type="checkbox" name="columnList[${vs.index}].isRes" value="1" ${column.isRes eq '1' ? 'checked' : ''}/>
									</td>
									<td>
